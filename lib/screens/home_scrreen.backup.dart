...
void dispose() {
  ...
  // Estruturas para Camadas (desenhando no Mapa)

  bool _modoDesenhoAtivo = false;
  List<Offset> _pontosDoDesenho = []; // Coordenadas da tela (pixels)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          // 1. CAMADA DE MAPA (Sempre ao fundo)
          GoogleMap(
            initialCameraPosition: CameraPosition(
              target: _pontoPartida?.location ?? LatLng(-23.5505, -46.6333),
              zoom: 15,
            ),
            onMapCreated: (controller) => _mapController = controller,
            polylines: _polylines,
            markers: _markers,
            // IMPORTANTE: Desativa o scroll do mapa enquanto desenha
            scrollGesturesEnabled: !_modoDesenhoAtivo,
          ),

          // 2. CAMADA DE DESENHO (Só aparece se o modo estiver ativo)
          if (_modoDesenhoAtivo)
            Positioned.fill(
              child: GestureDetector(
                onPanUpdate: (details) {
                  // Filtro para não carregar pontos demais (distância > 5px)
                  if (_pontosDoDesenho.isEmpty ||
                      (details.localPosition - _pontosDoDesenho.last).distance >
                          5) {
                    setState(() {
                      _pontosDoDesenho.add(details.localPosition);
                    });
                  }
                },
                onPanEnd: (details) {
                  _processarSelecaoPorZona();
                },
                child: CustomPaint(
                  painter: DesenhoPainter(_pontosDoDesenho),
                  size: Size.infinite,
                ),
              ),
            ),
      ...
      floatingActionButton: FloatingActionButton(
        backgroundColor: _modoDesenhoAtivo ? Colors.green : _corPrimaria,
        child: Icon(_modoDesenhoAtivo ? Icons.check : Icons.edit),
        onPressed: () {
          setState(() {
            _modoDesenhoAtivo = !_modoDesenhoAtivo;
            if (!_modoDesenhoAtivo) _pontosDoDesenho.clear();
          });
        },
      ),
    );
  }
  // Convertendo Pixels em Coordenadas Geográficas
  Future<void> _processarSelecaoPorZona() async {
    if (_mapController == null || _pontosDoDesenho.isEmpty) return;

    _notificar("Processando zona...", cor: Colors.blue);
    List<LatLng> areaGps = [];

    try {
      // PULO DO GATO: Não processamos TODOS os pontos. 
      // Pegamos um ponto a cada 5 para ganhar velocidade sem perder precisão.
      for (int i = 0; i < _pontosDoDesenho.length; i += 5) {
        Offset ponto = _pontosDoDesenho[i];
        
        // Adicionamos o '!' para garantir que o controller não é nulo
      ...
      // Garante que o último ponto também seja incluído para fechar o polígono
      if (_pontosDoDesenho.length % 5 != 0) {
        Offset ultimo = _pontosDoDesenho.last;
        LatLng latLngUltimo = await _mapController!.getLatLng(
          ScreenCoordinate(x: ultimo.dx.toInt(), y: ultimo.dy.toInt()),
        );
        areaGps.add(latLngUltimo);
      }

      // Chama a função de reordenar que usa o maps_toolkit
      
      void _reordenarPontosDentroDaArea(List<LatLng> poligonoGps) {
        if (poligonoGps.isEmpty) return;

        final LatLng pontoCentral = poligonoGps.first;
        final mt.LatLng pontoReferencia = mt.LatLng(
          pontoCentral.latitude,
          pontoCentral.longitude,
        );

        List<mt.LatLng> areaConvertida = poligonoGps
            .map((p) => mt.LatLng(p.latitude, p.longitude))
            .toList();

        List<DeliveryPoint> paradasSelecionadas = [];

        for (var parada in _paradasIntermediarias) {
          mt.LatLng pontoParada = mt.LatLng(
            parada.location.latitude,
            parada.location.longitude,
          );
          if (mt.PolygonUtil.containsLocation(
            ...
            paradasSelecionadas.add(parada);
          }
        }

        if (paradasSelecionadas.isNotEmpty) {
          paradasSelecionadas.sort((a, b) {
            double distA = mt.SphericalUtil.computeDistanceBetween(
              pontoReferencia,
              mt.LatLng(a.location.latitude, a.location.longitude),
            ).toDouble();
            double distB = mt.SphericalUtil.computeDistanceBetween(
              pontoReferencia,
              mt.LatLng(b.location.latitude, b.location.longitude),
            ).toDouble();
            return distA.compareTo(distB);
          });

          setState(() {
            _paradasIntermediarias.removeWhere(
              (p) => paradasSelecionadas.contains(p),
            );
            _paradasIntermediarias.insertAll(0, paradasSelecionadas);
          });

          _notificar("${paradasSelecionadas.length} paradas priorizadas.");
          _otimizarRotaComBaseNaNovaOrdem();
        }
      }

      _reordenarPontosDentroDaArea(areaGps);

    } catch (e) {
      _notificar("Erro ao converter coordenadas.", cor: Colors.red);
    } finally {
      // Limpa o desenho e volta para o modo normal
      setState(() {
        _pontosDoDesenho.clear();
        _modoDesenhoAtivo = false;
      });
    }
    // Exemplo simplificado de como você ordenaria os pontos
    void ordenarPorZona(List<LatLng> pontosDoCirculo, double raio) {
      List<DeliveryPoint> paradasNaZona = [];
      
      final LatLng pontoCentral = pontosDoCirculo.first;
      for (var parada in _paradasIntermediarias) {
        double distancia = calcularDistancia(pontoCentral, parada.location);
        if (distancia <= raio) {
          paradasNaZona.add(parada);
        }
      }

      setState(() {
        // Remove da lista original e reinsere na ordem de prioridade dos círculos
        _paradasIntermediarias.removeWhere((p) => paradasNaZona.contains(p));
        _paradasIntermediarias.addAll(paradasNaZona);
      });
  ...
  Widget _buildBarraBusca() {
    return Padding(
      ...
      child: Column(
        // Usamos Column para ter a busca em cima e os chips embaixo
        children: [
                ...
                child: TypeAheadField<Map<String, String>>(
                  ...
                  builder: (context, ctrl, node) => TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: "Buscar endereço...Ou Digite o CEP e Clique (+Parada)",
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(10),
                      ),
                    ),
                  ...
                  itemBuilder: (context, s) =>
                      ListTile(title: Text(s['descricao']!)),
                  onSelected: (s) =>
                      _solicitarNumeroEAtualizar(s['descricao']!),
                ),
              ),
              IconButton(
                icon: Icon(
          ...
          const SizedBox(height: 8), // Pequeno espaço entre a busca e os chips
          ...
          Row(
            ...
            children: [
              ...
              ChoiceChip(
                ...
                labelStyle: TextStyle(
                  color: _tipoSelecionado == "ENTREGA"
                      ? Colors.white
                      : Colors.black,
                ),
              ),
  ...
  Widget _buildBotoesAcao() {
    return Row(
      ...
      children: [
        _botaoPequeno(
          "INÍCIO",
          () => _adicionarPontoDireto(_searchController.text, "partida"),
          Colors.green,
        ),
        _botaoPequeno(
          "+ PARADA",
          () => _adicionarPontoDireto(_searchController.text, "parada"),
          _corPrimaria,
        ),
        _botaoPequeno(
          "FIM",
          () => _adicionarPontoDireto(_searchController.text, "destino"),
          Colors.red,
        ),
      ],
    );
  ...
  Widget _buildAreaMapa() {
    return Expanded(
      ...
      child: GoogleMap(
        initialCameraPosition: CameraPosition(
          target: _pontoPartida?.location ?? LatLng(-23.5505, -46.6333), // São Paulo como padrão
          zoom: 12,
        ),
        polylines: _polylines,
        markers: _markers,
